...
<header
    ...
    x-data
    @keydown.window.ctrl.q.prevent="let el = document.getElementById('navbar-search-input') || document.getElementById('navbar-search-input-v'); if(el) { el.focus(); el.select(); } else { window.location.href='{{ route('search') }}'; }">
        ...
        <div class="flex items-center relative h-16">
            ...
            <div class="shrink-0 lg:ml-0 flex items-center gap-x-5">
                ...
                <a class="" href="{{route('index')}}">
                    @if(config('settings.logo') && file_exists(public_path('static/img/'.config('settings.logo'))))
                        <img src="{{asset('static/img/'.config('settings.logo'))}}" class="w-auto h-7 object-contain"
                    ...
                    <form action="{{ route('search') }}" method="GET" class="w-full">
                        <div class="relative flex items-center">
                            <input type="search"
                                   ...
                                   name="q"
                                   value="{{ request('q') ?? (request()->routeIs('search') ? request()->route('search') : '') }}"
                                   placeholder="{{ __('Search') }} filmes, séries, animes..."
                                   class="w-full pl-12 pr-16 py-3 bg-gray-800/90 hover:bg-gray-800 focus:bg-gray-900 border border-gray-700/80 focus:border-primary-500/50 rounded-full text-sm text-white placeholder-gray-400 transition-all duration-200 outline-none focus:ring-2 focus:ring-primary-500/20"
                                   style="background-color: #1f2937 !important; color: #ffffff !important; caret-color: #8b5cf6 !important; -webkit-text-fill-color: #ffffff !important;"
                                   autocomplete="off" />
                            <button type="submit" aria-label="{{ __('Search') }}" class="absolute left-4 text-gray-400 hover:text-primary-400 transition-colors flex items-center justify-center">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/>
                                </svg>
                            </button>
                            <span class="absolute right-4 font-sans text-xxs text-gray-400 pointer-events-none hidden sm:block bg-gray-700/60 px-2 py-0.5 rounded">↵ Enter</span>
                        </div>
                    </form>
                </div>
            @endif
            <nav class="flex ml-auto rtl:mr-auto rtl:ml-0 lg:w-96">
                <ul class="flex grow justify-end flex-wrap items-center text-sm gap-x-4 sm:gap-x-6 lg:gap-x-7">

                    @if(isset($search) AND config('settings.layout') == 'vertical')
                        <li class="hidden 2xl:block">
                            <form action="{{ route('search') }}" method="GET" class="w-48 2xl:w-60">
                                <div class="relative flex items-center">
                                    <input type="search"
                                           id="navbar-search-input-v"
                                           name="q"
                                           value="{{ request('q') ?? (request()->routeIs('search') ? request()->route('search') : '') }}"
                                           placeholder="{{ __('Search') }}..."
                                           class="w-full pl-9 pr-4 py-2 bg-gray-800/90 hover:bg-gray-800 focus:bg-gray-900 border border-gray-700/80 focus:border-primary-500/50 rounded-full text-xs text-white placeholder-gray-400 transition-all duration-200 outline-none focus:ring-2 focus:ring-primary-500/20"
                                           style="background-color: #1f2937 !important; color: #ffffff !important; caret-color: #8b5cf6 !important; -webkit-text-fill-color: #ffffff !important;"
                                           autocomplete="off" />
                                    <button type="submit" aria-label="{{ __('Search') }}" class="absolute left-3 text-gray-400 hover:text-primary-400 transition-colors flex items-center justify-center">
                    ...
                    <!-- Ícone de Pesquisa no Cabeçalho (sempre visível em PC e Mobile) -->
                    @if(isset($search))
                        <li class="flex items-center">
                            <a href="{{ route('search') }}"
                               class="w-9 h-9 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800/80 rounded-xl transition-all duration-150"
                               title="{{ __('Search') }}">
