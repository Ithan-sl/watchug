    ...
    <div class="text-center">
        ...
        <a class="mx-auto inline-flex mb-5" href="{{route('index')}}">
            @if(config('settings.logo') && file_exists(public_path('static/img/'.config('settings.logo'))))
                <img src="{{asset('static/img/'.config('settings.logo'))}}" class="w-auto h-10 object-contain"
        ...
        <p class="text-gray-500 text-sm mt-4"> © {{date('Y')}} {{config('settings.site_name')}}. {{__('All rights reserved.')}}</p>
    </div>
    <!-- End Grid -->
</footer>
@else
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-12 gap-10 mb-10">
        <div class="col-span-full hidden lg:col-span-4 lg:block">
            ...
            <p class="mt-3 text-xs sm:text-sm text-gray-500 dark:text-gray-500">
                © {{date('Y')}} {{config('settings.site_name')}}. {{__('All rights reserved.')}}</p>
