<div class="mb-6" x-data="{stream:'0'}">
    @if(((in_array($listing->type, ['movie', 'tv']) AND $listing->member == 'active') OR (isset($listing->post->type) AND $listing->post->type == 'tv' AND $listing->post->member == 'active')) AND (auth()->check() && (empty(auth()->user()->plan_recurring_at) OR auth()->user()->plan_recurring_at < now()) OR !auth()->check()))
        <div
    ...
    @else
        ...
        @else
            ...
            @else
                <div class="-mx-3 lg:mx-0 relative group" id="player-container">
                    @php
                        ...
                        $playerBanner = \Illuminate\Support\Facades\Cache::remember('ad-6', 3600, function () {
                            return \App\Models\Advertisement::where('id', 6)->where('status', 'publish')->first();
                        });
                        ...
                        $showPlayerBanner = isset($playerBanner->body) && !empty(trim($playerBanner->body));
                        if ($showPlayerBanner && $playerBanner->user_hide == 'active' && auth()->check() && auth()->user()->plan_recurring_at > now()) {
                    ...
                    @if($showPlayerAd)
                        ...
                        @if($hasVisual)
                            <div x-data="{ adClosed: false, timeLeft: 5 }"
                                 x-show="!adClosed"
                                 x-init="let timer = setInterval(() => { if(timeLeft > 0) { timeLeft--; } else { clearInterval(timer); } }, 1000)"
                                 class="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/85 backdrop-blur-sm rounded-lg p-4 transition-all"
                                 style="display: flex;">
                                <div class="absolute top-3 right-3 z-40 flex items-center gap-2">
                                    <button @click="adClosed = true"
                                            type="button"
                                            class="px-3.5 py-1.5 bg-gray-800/90 hover:bg-gray-700 text-white text-xs font-semibold rounded-full border border-gray-600 transition flex items-center gap-1.5 shadow-xl cursor-pointer">
                                        <span x-show="timeLeft > 0" class="text-amber-400 font-mono" x-text="timeLeft + 's'"></span>
                                        <span x-text="timeLeft > 0 ? '{{__('Pular anúncio em')}}' : '{{__('Fechar anúncio')}} ✕'"></span>
                                    </button>
                                </div>
                ...
                @if(isset($showPlayerBanner) && $showPlayerBanner)
                    <div class="my-4 text-center">
                        {!! html_entity_decode($playerBanner->body) !!}
                    </div>
                @endif
                @if(count($videos)>1)
                    <div class="my-4 text-center">
                            ...
                            @foreach($videos as $key => $video)
                                <li>
                                    <button
                                        ...
                                        x-on:click="stream = '{{$loop->index}}'; document.getElementById('video-iframe').src = 'about:blank'; setTimeout(() => { document.getElementById('video-iframe').src = '{{$video['link']}}'; }, 50);">
