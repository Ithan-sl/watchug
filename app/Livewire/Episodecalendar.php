...
class Episodecalendar extends Component
{
    ...
    public function render()
    {
        ...
        if($this->day) {
            $listings = PostEpisode::where('status','publish')->has('post')->has('season')->with(['post', 'season'])->whereDate('created_at',$this->day)->limit($limit ?? 24)->orderby('id', 'desc')->get();
        } else {
            $listings = PostEpisode::where('status','publish')->has('post')->has('season')->with(['post', 'season'])->whereDate('created_at',now()->format('Y-m-d'))->limit($limit ?? 24)->orderby('id', 'desc')->get();
        }
