...
class PostFilter extends Component
{
    ...
    public function render()
    {
        ...
        $listings = $listings->where('status','publish')->with(['genres']);
        ...
        $recommends = Cache::remember('browse-recommends', 3600, function () {
            return Post::with(['genres'])->where('status', 'publish')->orderby('vote_average','desc')->limit(9)->get();
        });
        $filterGenres = Cache::rememberForever('filter-genres-all', function () {
            return Genre::all();
        });
        $filterCountries = Cache::rememberForever('filter-countries-all', function () {
            return Country::where('filter', 'active')->get();
        });
