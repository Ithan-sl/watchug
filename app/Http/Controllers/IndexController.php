...
class IndexController extends Controller
{
    public function index()
    {
        ...
        $modules = Cache::rememberForever('home-modules', function () {
            return Module::where('status', 'active')->orderby('sortable', 'asc')->get();
        });
        ...
        foreach ($modules as $module) {
            ...
            if($module->slug == 'slider') {
                $listings['slider'] = Cache::rememberForever('home-slider', function () use ($limit) {
                    return Post::with(['country', 'genres'])->where('slider', 'active')->where('status','publish')->orderBy('id', 'desc')->limit($limit)->get();
                });
            } elseif($module->slug == 'movie') {
                $listings['movie'] = Cache::rememberForever('home-movie', function () use ($limit) {
                    return Post::with(['genres'])->where('type', 'movie')->where('status', 'publish')->orderBy('id', 'desc')->limit($limit ?? 16)->get();
                });
            } elseif($module->slug == 'tv') {
                $listings['tv'] = Cache::rememberForever('home-tv', function () use ($limit) {
                    return Post::with(['genres'])->where('type', 'tv')->where('status','publish')->orderby('id','desc')->limit($limit ?? 16)->get();
                });
            } elseif($module->slug == 'episode') {
                $listings['episode'] = Cache::rememberForever('home-episode', function () use ($limit) {
                    return PostEpisode::where('status','publish')->has('post')->has('season')->with(['post', 'season'])->orderby('id','desc')->limit($limit ?? 16)->get();
                });
                ...
                $listings['genres'] = Cache::rememberForever('home-genre', function () {
                    return Genre::withCount(['posts'])->where('featured', 'active')->limit(5)->get();
                });
            } elseif($module->slug == 'collection') {
                $listings['collection'] = Cache::rememberForever('home-collection', function () {
    ...
    public function search(Request $request) {
        if ($request->filled('q')) {
            return redirect()->route('search', ['search' => $request->q]);
        }
