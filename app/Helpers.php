...
if (!function_exists('picture')) {
    ...
    {
        if (empty($image)) {
            $image = '/static/img/placeholder/300.png';
        } elseif (is_string($image) && (str_starts_with($image, 'http://127.0.0.1:8000/') || str_starts_with($image, 'http://localhost:8000/'))) {
            $image = preg_replace('#^http://(127\.0\.0\.1|localhost):8000/#', '/', $image);
        }
        ...
        $webpSource = (!$isExternal && function_exists('webper')) ? '<source data-srcset="' . webper($image) . '" type="image/webp" class="' . $class . '">' : '';

        return '<picture>' . $webpSource . '<img src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="' . $image . '" alt="' . $titleEscaped . '" class="lazyload ' . $class . '" ' . $sizeHtml . ' loading="lazy" decoding="async"></picture>';
    }
}
