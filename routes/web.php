...
Route::get('pesquisa/{search?}', [\App\Http\Controllers\SearchController::class, 'index'])->name('pesquisa');
...
Route::get(__('embed') . '-server', [App\Http\Controllers\WatchController::class, 'embedServer'])->name('embed.server')->middleware('hotlink');

// 1. Extração Direta e Proxy Reverso de Vídeo HLS (.m3u8 / .ts) com Token Criptografado
Route::get('api/stream/manifest.m3u8', [\App\Http\Controllers\StreamProxyController::class, 'manifest'])->name('stream.manifest');
Route::get('api/stream/segment.ts', [\App\Http\Controllers\StreamProxyController::class, 'segment'])->name('stream.segment');
